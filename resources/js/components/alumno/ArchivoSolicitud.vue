<template>
    ...
    <v-toolbar flat color="white">
      <v-toolbar-title>Archivo Solicitud</v-toolbar-title>
      <v-divider
        class="mx-2"
        inset
        vertical
    ...
    methods: {
      ...
      deleteItem (item) {
        const index = this.desserts.indexOf(item)
          this.editedItem = Object.assign({}, item)
        if (confirm('¿Seguro que quieres eliminar esta Solicitud?') && this.desserts.splice(index, 1)) {
            axios.delete('http://192.168.10.10/Solicitud/'+this.editedItem.codigo, {
            }).then(response => {
